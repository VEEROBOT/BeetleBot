...
check_ros_network() {
    ...
    # Check if we can discover nodes
    if ! timeout 2 ros2 node list &>/dev/null; then
...
check_bridge_health() {
    ...
    # Check if we can call services
    if ! timeout 2 ros2 service call /lyra/arm std_srvs/srv/Trigger --no-display-reply &>/dev/null 2>&1; then
        print_error "Cannot reach bridge services"
        print_recommendation "Bridge may be hung or unresponsive"
        return 1
    fi
    
    print_success "Bridge is responding to service calls"
    return 0
}
...
check_telemetry_flow() {
    ...
    if ! timeout 2 ros2 topic echo /battery_voltage --once &>/dev/null 2>&1; then
    ...
    if ! timeout 2 ros2 topic echo /wheel_rpm --once &>/dev/null 2>&1; then
    ...
    if ! timeout 2 ros2 topic echo /lyra/armed --once &>/dev/null 2>&1; then
...
check_battery_level() {
    ...
    local voltage=$(timeout 2 ros2 topic echo /battery_voltage --once 2>/dev/null | grep "data:" | awk '{print $2}')
...
check_odometry() {
    ...
    # Check wheel ticks
    if ! timeout 2 ros2 topic echo /wheel_ticks --once &>/dev/null 2>&1; then
...
check_imu() {
    ...
    if ! timeout 2 ros2 topic echo /imu/data_raw --once &>/dev/null 2>&1; then
...
check_lidar() {
    ...
    local freq=$(timeout 3 ros2 topic hz /scan 2>&1 | grep "average frequency" | awk '{print $3}')
